...
reduced_train_df = train_df.drop(labels=['id'], axis = 1)

reduced_test_df = test_df.drop(labels=['id'], axis = 1)

y_train = reduced_train_df["Exited"]
...
class ChurnClassifier2H(nn.Module):
    ...
    def __init__(self, input_size, dropout=0.2):
        super().__init__()
        self.block1 = ChurnClassifier2HBlock(input_size, dropout)
        self.block2 = ChurnClassifierConvWrapper(input_size, conv_channels=15)
        self.block3 = ChurnClassifier2HBlock(input_size, dropout)
        self.block4 = ChurnClassifierConvWrapper(input_size, conv_channels=15)
        self.block5 = ChurnClassifier2HBlock(input_size, dropout)
...
model = ChurnClassifier2H(input_size)
...
# Initialize the model for each fold
model = ChurnClassifier2H(X.shape[1])
...
test_df_result.to_csv("NN_3B_Adam_CyclingLR_Model_probs.csv", index=False)
