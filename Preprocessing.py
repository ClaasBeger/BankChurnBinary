...
# Potentially investigate information loss through coordinate encoding

#%%

# Further normalization
# Apply Zscore standardisation to CreditScore and add additional column with pre-defined credit range label
...
train_df['CreditScore_Range'] = train_df["CreditScore"].map(lambda x: assignLabel(x))

zscore_scaler = preprocessing.StandardScaler().fit(train_df[['CreditScore']])

#Apply the Zscore scaler Fare attribute and assign to a new Zscore column
train_df['CreditScore_zscore']=zscore_scaler.transform(train_df[['CreditScore']])

train_df.drop('CreditScore', axis=1, inplace=True)

#%%

# Apply Zscore to Age
zscore_scaler = preprocessing.StandardScaler().fit(train_df[['Age']])

#Apply the Zscore scaler Fare attribute and assign to a new Zscore column
train_df['Age_zscore']=zscore_scaler.transform(train_df[['Age']])

train_df.drop('Age', axis=1, inplace=True)
# Apply Zscore to Balance
zscore_scaler = preprocessing.StandardScaler().fit(train_df[['Balance']])

#Apply the Zscore scaler Fare attribute and assign to a new Zscore column
train_df['Balance_zscore']=zscore_scaler.transform(train_df[['Balance']])

train_df.drop('Balance', axis=1, inplace=True)
# Apply Zscore to Estimated Salary
zscore_scaler = preprocessing.StandardScaler().fit(train_df[['EstimatedSalary']])

train_df['EstimatedSalary_zscore']=zscore_scaler.transform(train_df[['EstimatedSalary']])

train_df.drop('EstimatedSalary', axis=1, inplace=True)

#%%

train_df.to_csv("test_cleaned.csv", index=False)